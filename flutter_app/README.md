...
Set `DITTO_DATABASE_ID` and `DITTO_OFFLINE_LICENSE_TOKEN` in `flutter_app/.env`
to run this app in offline-only mode (peer-to-peer only, no cloud sync). When
the offline token is non-empty, `DITTO_DEVELOPMENT_TOKEN` and `DITTO_SERVER_URL`
are not used. Request a token from <support@ditto.com>. See the top-level
