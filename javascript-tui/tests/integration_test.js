...
async function createDittoInstance() {
  ...
  const databaseId = process.env.DITTO_DATABASE_ID;
  const token = process.env.DITTO_DEVELOPMENT_TOKEN;
  const serverURL = process.env.DITTO_SERVER_URL;
  ...
  const config = new DittoConfig(databaseId, connectConfig, tempdir);
