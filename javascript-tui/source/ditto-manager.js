...
export class DittoManager {
  constructor({
    ...
    this.offlineLicenseToken = offlineLicenseToken.trim();
  ...
  async open() {
    const connectConfig = this.offlineLicenseToken
      ? { mode: 'smallPeersOnly' }
      : { mode: 'server', url: this.serverURL };
    ...
    if (this.offlineLicenseToken) {
      ditto.setOfflineOnlyLicenseToken(this.offlineLicenseToken);
    } else {
