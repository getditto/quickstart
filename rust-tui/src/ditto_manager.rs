...
use dittolive_ditto::{Ditto, fs::TempRoot, prelude::*};
...
impl DittoManager {
    ...
    pub fn try_new(
        ...
        let offline_license_token = offline_license_token.trim().to_owned();
        let connect_config = if offline_license_token.is_empty() {
        ...
        if offline_license_token.is_empty() {
            ditto
                .auth()
