...
func NewDittoManager(databaseId, token, serverURL string) (*DittoManager, error) {
	...
	// Initialize Ditto with the Server connection API
	config := ditto.DefaultDittoConfig().
		WithDatabaseID(databaseId).
		WithPersistenceDirectory(tempDir).
		WithConnect(&ditto.DittoConfigConnectServer{URL: serverURL})
