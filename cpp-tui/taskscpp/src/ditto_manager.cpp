...
init_ditto(string database_id, string development_token, string server_url,
           string offline_license_token, string persistence_dir) {
  try {
    const auto first_non_whitespace =
        offline_license_token.find_first_not_of(" \t\r\n");
    const auto is_offline = first_non_whitespace != string::npos;
    if (is_offline) {
      const auto last_non_whitespace =
          offline_license_token.find_last_not_of(" \t\r\n");
      offline_license_token = offline_license_token.substr(
          first_non_whitespace, last_non_whitespace - first_non_whitespace + 1);
    } else {
      offline_license_token.clear();
    }

    auto config = ditto::DittoConfig::default_config();
    config.set_database_id(std::move(database_id))
        .set_persistence_directory(std::move(persistence_dir));
    if (is_offline) {
      config.set_small_peers_only_connect();
    } else {
      config.set_server_connect(std::move(server_url));
    }

    auto ditto = ditto::Ditto::open(std::move(config));

    if (is_offline) {
      ditto->set_offline_only_license_token(std::move(offline_license_token));
    } else if (auto auth = ditto->get_auth()) {
...
public:
  ...
  Impl(string database_id, string development_token, string server_url,
      ...
      : ditto(init_ditto(std::move(database_id), std::move(development_token),
                         std::move(server_url),
                         std::move(offline_license_token),
