...
// This module maintains a singleton C++ TasksPeer instance which performs all
// the Ditto-related functions, and all methods operate on that singleton.  The
// mutex must be locked by any thread that is accessing the peer.
...
jobject native_task_to_java_task(JNIEnv *const env, const Task &native_task) {
  ...
  jmethodID ctor = env->GetMethodID(
      taskClass, "<init>", "(Ljava/lang/String;Ljava/lang/String;ZZ)V");
  if (ctor == nullptr) {
  ...
  auto java_task =
      env->NewObject(taskClass, ctor, id.get(), title.get(), done, deleted);
  return java_task;
}
...
Java_live_ditto_quickstart_tasks_TasksLib_initDitto(
    JNIEnv *env, jobject thiz, jobject context, jstring app_id, jstring token,
    jstring persistence_dir, jboolean is_running_on_emulator,
    jstring custom_auth_url, jstring websocket_url) {
  __android_log_print(ANDROID_LOG_DEBUG, TAG,
                      "Java_live_ditto_quickstart_tasks_TasksLib_initDitto; "
                      "SDK version: %s; emulator: %s",
                      TasksPeer::get_ditto_sdk_version().c_str(),
                      is_running_on_emulator ? "true" : "false");
  try {
    std::lock_guard<std::recursive_mutex> lock(mtx);
    if (peer) {
      throw_java_illegal_state_exception(
          env, "cannot call initDitto multiple times");
      return;
    }
    ...
    peer = std::make_shared<TasksPeer>(
        env, context, std::move(app_id_str), std::move(token_str), true,
        std::move(persistence_dir_str), is_running_on_emulator,
        std::move(custom_auth_url_str), std::move(websocket_url_str));

  } catch (const std::exception &err) {
    __android_log_print(ANDROID_LOG_ERROR, TAG, "initDitto failed: %s",
                        err.what());
...
extern "C" JNIEXPORT void JNICALL
Java_live_ditto_quickstart_tasks_TasksLib_terminateDitto(JNIEnv *env,
                                                         jobject thiz) {
  __android_log_print(
      ANDROID_LOG_DEBUG, TAG,
      "Java_live_ditto_quickstart_tasks_TasksLib_terminateDitto");
  try {
    std::lock_guard<std::recursive_mutex> lock(mtx);
    if (!peer) {
      throw_java_illegal_state_exception(env,
                                         "TasksLib has not been initialized");
      return;
    }
    remove_observer(env);
    peer.reset();
  } catch (const std::exception &err) {
    __android_log_print(ANDROID_LOG_ERROR, TAG, "terminateDitto failed: %s",
                        err.what());
    throw_java_exception(env, err.what());
  }
}
...
Java_live_ditto_quickstart_tasks_TasksLib_isSyncActive(JNIEnv *env,
                                                       jobject thiz) {
  __android_log_print(ANDROID_LOG_DEBUG, TAG,
  ...
  try {
    ...
    __android_log_print(ANDROID_LOG_ERROR, TAG, "isSyncActive failed: %s",
                        err.what());
    throw_java_exception(env, err.what());
    return JNI_FALSE;
  }
...
Java_live_ditto_quickstart_tasks_TasksLib_startSync(JNIEnv *env, jobject thiz) {
  ...
  try {
    ...
    __android_log_print(ANDROID_LOG_ERROR, TAG, "startSync failed: %s",
                        err.what());
    throw_java_exception(env, err.what());
  }
}
...
Java_live_ditto_quickstart_tasks_TasksLib_stopSync(JNIEnv *env, jobject thiz) {
  __android_log_print(ANDROID_LOG_DEBUG, TAG,
                      "Java_live_ditto_quickstart_tasks_TasksLib_stopSync");
  try {
    std::lock_guard<std::recursive_mutex> lock(mtx);
    if (!peer) {
      throw_java_illegal_state_exception(env,
    ...
    __android_log_print(ANDROID_LOG_ERROR, TAG, "stopSync failed: %s",
                        err.what());
    throw_java_exception(env, err.what());
  }
}
...
Java_live_ditto_quickstart_tasks_TasksLib_getTaskWithId(JNIEnv *env,
                                                        jobject thiz,
                                                        jstring task_id) {
  __android_log_print(
      ANDROID_LOG_DEBUG, TAG,
  ...
  try {
    ...
    __android_log_print(ANDROID_LOG_ERROR, TAG, "getTaskWithId failed: %s",
                        err.what());
    throw_java_exception(env, err.what());
    return nullptr;
  }
...
extern "C" JNIEXPORT void JNICALL
Java_live_ditto_quickstart_tasks_TasksLib_createTask(JNIEnv *env, jobject thiz,
                                                     jstring title,
                                                     jboolean done) {
  __android_log_print(ANDROID_LOG_DEBUG, TAG,
  ...
  try {
    ...
    __android_log_print(ANDROID_LOG_ERROR, TAG, "createTask failed: %s",
                        err.what());
    throw_java_exception(env, err.what());
  }
}
...
extern "C" JNIEXPORT void JNICALL
Java_live_ditto_quickstart_tasks_TasksLib_updateTask(JNIEnv *env, jobject thiz,
                                                     jstring task_id,
                                                     jstring title,
                                                     jboolean done) {
  __android_log_print(ANDROID_LOG_DEBUG, TAG,
  ...
  try {
    ...
    Task task(jstring_to_string(env, task_id), jstring_to_string(env, title),
              done);
    peer->update_task(task);
  } catch (const std::exception &err) {
    __android_log_print(ANDROID_LOG_ERROR, TAG, "updateTask failed: %s",
                        err.what());
    throw_java_exception(env, err.what());
  }
}
...
extern "C" JNIEXPORT void JNICALL
Java_live_ditto_quickstart_tasks_TasksLib_deleteTask(JNIEnv *env, jobject thiz,
                                                     jstring task_id) {
  __android_log_print(ANDROID_LOG_DEBUG, TAG,
  ...
  try {
    ...
    __android_log_print(ANDROID_LOG_ERROR, TAG, "deleteTask failed: %s",
                        err.what());
    throw_java_exception(env, err.what());
  }
}
...
extern "C" JNIEXPORT void JNICALL
Java_live_ditto_quickstart_tasks_TasksLib_toggleDoneState(JNIEnv *env,
                                                          jobject thiz,
                                                          jstring task_id) {
  __android_log_print(
      ANDROID_LOG_DEBUG, TAG,
  ...
  try {
    ...
    __android_log_print(ANDROID_LOG_ERROR, TAG, "toggleDoneState failed: %s",
                        err.what());
    throw_java_exception(env, err.what());
  }
}
...
extern "C" JNIEXPORT void JNICALL
Java_live_ditto_quickstart_tasks_TasksLib_insertInitialDocuments(JNIEnv *env,
                                                                 jobject thiz) {
  __android_log_print(
      ANDROID_LOG_DEBUG, TAG,
  ...
  try {
    ...
    __android_log_print(ANDROID_LOG_ERROR, TAG,
                        "insertInitialDocuments failed: %s", err.what());
    throw_java_exception(env, err.what());
  }
}

extern "C" JNIEXPORT void JNICALL
Java_live_ditto_quickstart_tasks_TasksLib_setTasksObserver(JNIEnv *env,
                                                           jobject thiz,
                                                           jobject observer) {
  __android_log_print(
      ANDROID_LOG_DEBUG, TAG,
  ...
  try {
    ...
    if (javaTasksObserver != nullptr || tasksStoreObserver != nullptr) {
      throw_java_illegal_state_exception(env,
                                         "a tasks observer is already set");
      return;
    }
    ...
    tasksStoreObserver = peer->register_tasks_observer(
        [vm](const std::vector<std::string> &tasksJson) {
          try {
            __android_log_print(ANDROID_LOG_DEBUG, TAG,
                                "tasks observer callback invoked");
            std::lock_guard<std::recursive_mutex> lock(mtx);
            if (javaTasksObserver == nullptr) {
              return;
            }
            ...
            TempLocalRef<jobjectArray> stringArray(
                env, strings_to_jstrings(env, tasksJson));

            // Invoke the onTasksUpdated method of the Java observer
            TempLocalRef<jclass> observerClass(
                env, env->GetObjectClass(javaTasksObserver));
            jmethodID methodID =
                env->GetMethodID(observerClass.get(), "onTasksUpdated",
                                 "([Ljava/lang/String;)V");
            if (methodID == nullptr) {
              throw std::runtime_error(
                  "unable to get method ID for onTasksUpdated of observer");
            }
            env->CallVoidMethod(javaTasksObserver, methodID, stringArray.get());
          } catch (const std::exception &err) {
            __android_log_print(ANDROID_LOG_ERROR, TAG,
                                "error processing tasks update: %s",
                                err.what());
          }
        });
  } catch (const std::exception &err) {
    __android_log_print(ANDROID_LOG_ERROR, TAG, "setTasksObserver failed: %s",
                        err.what());
    throw_java_exception(env, err.what());
  }
}
...
extern "C" JNIEXPORT void JNICALL
Java_live_ditto_quickstart_tasks_TasksLib_removeTasksObserver(JNIEnv *env,
                                                              jobject thiz) {
  __android_log_print(
      ANDROID_LOG_DEBUG, TAG,
  ...
  try {
    ...
    __android_log_print(ANDROID_LOG_ERROR, TAG,
                        "removeTasksObserver failed: %s", err.what());
    throw_java_exception(env, err.what());
  }
}
