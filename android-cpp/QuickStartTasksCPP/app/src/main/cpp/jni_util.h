...
///
/// The caller is responsible for eventually calling env->DeleteLocalRef() on
/// the returned array.
jobjectArray strings_to_jstrings(JNIEnv *env,
                                 const std::vector<std::string> &strings);

/// Takes ownership of a local reference to a Java object, and releases it upon
/// destruction
template <class T> class TempLocalRef {
public:
  /// Take ownership of the specified local reference.
  ///
  /// The env must be the same object that created the local reference, and must
  /// remain valid until this object's destructor is called.
  TempLocalRef(JNIEnv *env, T localRef) noexcept
      : env(env), localRef(localRef) {}
...
/// Converts a C++ string to an owned `jstring`, which will be freed upon
/// destruction.
class TempJString {
public:
  ...
  TempJString(JNIEnv *env, const char *c_str)
      : js(env, env->NewStringUTF(c_str)) {
    if (js.get() == nullptr) {
  ...
  ///
  /// The `env` pointer must remain valid until the destructor has been called.
  TempJString(JNIEnv *env, const std::string &s)
      : TempJString(env, s.c_str()) {}

  /// Return converted `jstring`
  ///
  /// The returned object is only valid until this object is destroyed.
  [[nodiscard]] jstring get() const noexcept { return js.get(); }
...
public:
  ...
  [[nodiscard]] JNIEnv *env() const noexcept { return thread_env; }
